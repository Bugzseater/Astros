        ...
        {planets.map((planet, index) => (
          <div key={planet.name} className="flex flex-col items-center group">
            ...
            <div className="relative mb-8">
              <div className={`${planet.size} relative cursor-pointer transform transition-all duration-500 group-hover:scale-110 group-hover:rotate-12`}>
                {/* Planet Image */}
                <img
                  src={planet.image}
                  alt={planet.name}
                  className={`w-full h-full object-cover rounded-full ${planet.shadow} shadow-2xl`}
                  style={{
              ...
              {/* Planet Glow */}
              <div className={`absolute inset-0 ${planet.size} rounded-full bg-gradient-radial from-current to-transparent opacity-20 blur-2xl -z-10 ${planet.glowColor}`} />
            </div>
            ...
            <div className="text-center max-w-xs flex justify-center flex-col items-center">
              ...
              <button className="flex items-center justify-center space-x-2 text-white hover:text-blue-400 transition-colors duration-300 group-hover:scale-105 transform">
